...
int main() {
  ...
  // extracting address from client addr
  char ipstr[NI_MAXHOST];
  char portstr[NI_MAXSERV];

  int rc = getnameinfo((struct sockaddr *)&client_addr, client_addr_len, ipstr,
                       sizeof(ipstr), portstr, sizeof(portstr),
                       NI_NUMERICHOST | NI_NUMERICSERV);

  printf("[INFO] Client connected to the server. (%s : %s)\n", ipstr, portstr);
  while (1) {
